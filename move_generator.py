...
class MoveGenerator:
    ...
    def generate_pawn_moves(self, pawn, start_position, board_state):
        ...
        # # En Passant
        # if board_state.last_move and Piece.is_pawn(board_state.last_move.piece):
        #     from_row, from_col = board_state.last_move.start
        #     to_row, to_col = board_state.last_move.end
        #     if abs(from_row - to_row) == 2 and from_col == to_col:  # Check for two-square pawn move
        #         if to_row == start_row and abs(to_col - start_col) == 1:  # Pawn moved adjacent to the current pawn
        #             en_passant_row = forward_one  # The row the pawn would move to capture en passant
        #             en_passant_col = to_col  # The column of the captured pawn
    ...
    #     # Forward moves
    #     if 0 <= forward_one < 8 and board_state.board[forward_one][start_col] == Piece.No_Piece:
    #         moves.append((forward_one, start_col))
    #         if start_row in (1, 6) and board_state.board[forward_two][start_col] == Piece.No_Piece:
    #             moves.append((forward_two, start_col))
    ...
    # # En Passant logic
    #     if board_state.last_move and Piece.is_pawn(board_state.last_move.piece):
    #         print ("last move is:", board_state.last_move)
    #         from_row, from_col = board_state.last_move.start
